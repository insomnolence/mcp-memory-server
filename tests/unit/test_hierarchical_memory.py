...
from src.mcp_memory_server.memory import HierarchicalMemorySystem
...
def hierarchical_memory_system(mock_short_term_collection, mock_long_term_collection, mock_permanent_collection):
    ...
    # Mock external dependencies - using new service facade path
    with patch('src.mcp_memory_server.memory.services.facade.Chroma', autospec=True) as MockChroma:
        with patch('src.mcp_memory_server.memory.services.facade.HuggingFaceEmbeddings', autospec=True):
            with patch(
                'src.mcp_memory_server.memory.services.facade.MemoryImportanceScorer',
                autospec=True
            ) as MockScorer:
                with patch(
                    'src.mcp_memory_server.memory.services.facade.MemoryDeduplicator',
                    autospec=True
                ) as MockDeduplicator:
                    facade = 'src.mcp_memory_server.memory.services.facade'
                    with patch(f'{facade}.QueryPerformanceMonitor', autospec=True):
                        with patch(f'{facade}.MemoryIntelligenceSystem', autospec=True):
                            with patch(
                                'src.mcp_memory_server.memory.services.facade.ChunkRelationshipManager',
                                autospec=True
                            ) as MockChunkManager:
                                ...
                                mock_deduplicator_instance.check_ingestion_duplicates = Mock(
                                    return_value=('add', None, 0.0))
                                mock_deduplicator_instance.boost_existing_document = Mock(
