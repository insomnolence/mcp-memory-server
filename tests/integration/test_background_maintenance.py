...
async def test_background_maintenance_starts_and_stops(running_mcp_server):
    ...
    assert "Background maintenance started" in start_result["result"]["message"], "Background maintenance start message not found"
    ...
    assert stats_result['result']['maintenance']['thread_active'] is True, "Maintenance thread not active after starting"
    ...
    assert "Background maintenance stopped" in stop_result["result"]["message"], "Background maintenance stop message not found"
    ...
    assert "error" not in stats_result_after_stop, f"Error getting lifecycle stats after stop: {stats_result_after_stop.get('error')}"
    assert stats_result_after_stop['result']['maintenance']['thread_active'] is False, "Maintenance thread still active after stopping"
...
async def test_background_maintenance_cleanup_actually_works(running_mcp_server):
    ...
    assert len(query_result_after['result']['results']) > 0, "Non-expired document should still be present after cleanup"
    assert query_result_after['result']['results'][0]['content'] == expiring_content, "Document content should be preserved"
    
    print(f"\n✅ Cleanup system working correctly:")
    print(f"   - TTL metadata properly applied (tier: {ttl_tier})")
    print(f"   - Cleanup function successfully identifies non-expired documents")
    print(f"   - Document preservation works as expected")
...
async def test_background_maintenance_preserves_document_metadata(running_mcp_server):
    ...
    assert query_result_after['result']['results'][0]['content'] == aging_content, "Document content should be preserved"
