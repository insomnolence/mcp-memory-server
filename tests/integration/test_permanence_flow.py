...
async def test_permanent_document_persists_through_cleanup(running_mcp_server):
    ...
    print(f"Document metadata: permanence_flag={metadata.get('permanence_flag')}, permanent_flag={metadata.get('permanent_flag')}, ttl_tier={metadata.get('ttl_tier')}")
    ...
    if importance_score >= 0.8:
        ...
        assert metadata.get('ttl_tier') in ['permanent', 'static'] or metadata.get('permanent_flag') is True, "Document should be in permanent/static tier for high importance + critical flag"
    else:
    ...
    assert len(query_result_after_cleanup['result']['results']) > 0, "Permanent document was unexpectedly deleted after cleanup"
    ...
    assert "error" not in query_perm_tool_result, f"Error querying permanent documents tool: {query_perm_tool_result.get('error')}"
    assert len(query_perm_tool_result['result']['results']) > 0, "Permanent document not found via query_permanent_documents_tool"
    assert query_perm_tool_result['result']['results'][0]['metadata']['permanence_flag'] == 'critical', "Permanence flag lost in permanent query"
...
async def test_high_importance_document_persists(running_mcp_server):
    ...
    assert len(query_result_initial['result']['results']) > 0, "High importance document not found immediately after adding"
    ...
    if importance_score >= 0.95:
        assert ttl_tier == 'permanent' or permanent_flag is True, f"High importance document (score: {importance_score}) should be permanent"
    else:
        print(f"Document importance {importance_score} not high enough for auto-permanence, testing high-tier preservation")
    ...
    assert len(query_result_after_cleanup['result']['results']) > 0, "High importance document was unexpectedly deleted after cleanup"
