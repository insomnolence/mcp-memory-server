...
class QueryPerformanceMonitor:
    ...
    def track_query(self, query: str, results: dict, processing_time: float, 
        ...
        logging.debug(f"Tracked query {query_id}: {processing_time*1000:.1f}ms, {query_record['total_results']} results")
    ...
    def _calculate_query_metrics(self, query_record: dict, results: dict):
        ...
        query_record['collections_efficiency'] = query_record['total_results'] / max(len(query_record['collections_searched']), 1)
    ...
    def _update_aggregated_stats(self, query_record: dict):
        ...
        self.hourly_stats[hour_key]['dedup_hit_rates'].append(query_record['deduplication_impact']['deduplication_hit_rate'])
        ...
        self.daily_stats[day_key]['dedup_hit_rates'].append(query_record['deduplication_impact']['deduplication_hit_rate'])
    ...
    def get_performance_summary(self, time_window: str = 'all') -> Dict[str, Any]:
        ...
        summary = {
            ...
            'quality_metrics': {
                ...
                'high_quality_queries': len([q for q in quality_scores if q > self.config['performance_thresholds']['good_result_quality']])
            },
            ...
            'optimization_usage': {
                'smart_routing_usage': len([q for q in filtered_queries if q['smart_routing_used']]) / len(filtered_queries) * 100,
                'query_optimization_usage': len([q for q in filtered_queries if q['query_optimization_applied']]) / len(filtered_queries) * 100
            }
        }
