...
def get_memory_stats_tool(memory_system: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        stats_text = _format_stats_as_text(stats)
...
def _format_stats_as_text(stats: Dict[str, Any]) -> str:
    ...
    enhanced = stats.get('enhanced_metrics', {})
    lines.append(f"**Total Documents:** {enhanced.get('total_documents', 0)}")
    lines.append("**Storage Efficiency:** {:.1f}%".format(enhanced.get('storage_efficiency', 0)))
    lines.append("**System Optimization Score:** {:.2f}\n".format(enhanced.get('system_optimization_score', 0)))

    lines.append("## Collections:")
    for name, collection in stats.get('collections', {}).items():
        if isinstance(collection, dict):
            lines.append(f"  - **{name}**: {collection.get('count', 0)} documents")

    dedup = stats.get('deduplication', {})
    if dedup.get('enabled'):
        lines.append("\n## Deduplication:")
        lines.append(f"  - **Efficiency:** {dedup.get('deduplication_efficiency', 0):.1f}%")
        lines.append(f"  - **Documents Merged:** {dedup.get('total_documents_merged', 0)}")
...
def get_system_health_tool(memory_system: Any, config: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Return MCP-compliant format
        return create_success_response(
            message=f"System health: {status} (score: {overall_health_score:.2f})",
            data={
        ...
        return create_tool_error(
            f"Failed to assess system health: {str(e)}",
            MCPErrorCode.MEMORY_SYSTEM_ERROR,
            original_error=e
        )
